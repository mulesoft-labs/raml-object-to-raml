...
/**
 * Check if the value is a primitive type (can be an instance of String, etc.)
 *
 * @param  {*}       value
 * @return {Boolean}
 */
...
var arrayWithinLength = function (array, maxLength, opts) {
  // Arrays with only one property must always be true.
  if (!array.length) {
    return true;
  }
  ...
  return array.every(function (value) {
    ...
    total += stringify(value, 0, opts).length;
...
var stringWithinLength = function (str, maxLength, opts) {
  ...
  return stringifyString(str).length < maxLength;
};
...
var stringifyProperty = function (prefix, value, level, opts) {
  var maxLength = opts.maxLength - prefix.length - 1;

  // Check whether the array can fit using inline representation.
  if (Array.isArray(value) && arrayWithinLength(value, maxLength, opts)) {
    return prefix + ' ' + stringifyArrayInline(value, level + 1, opts);
  }

  // Check whether strings should be on a single line.
  if (isString(value) && !stringWithinLength(value, maxLength, opts)) {
